...
_PLATFORM_MAP = build_short_platform_map(bazel_runfile("ray-images.json"))
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            ...
            "anyscalecudabuild--platformcu1300cudnn-python312",
        ),
        # ray-llm
        (
        ...
        (
            ...
            "anyscalecudabuild--platformcu999-python310",
        ),
        # No python version → bare config key (cpu assumed for ray without suffix)
...
def test_get_prerequisite_step(image, base_image, expected):
    assert get_prerequisite_step(image, base_image, _PLATFORM_MAP) == expected


def test_short_platform_map_built_from_ray_images_json():
    """The map built from ray-images.json has no short-tag collisions."""
    ray_images_path = str(Path(bazel_runfile("ray-images.json")))
    platform_map = build_short_platform_map(ray_images_path)
    assert "cpu" in platform_map
    # Verify at least one CUDA platform is present.
    cuda_entries = {k: v for k, v in platform_map.items() if k.startswith("cu")}
    assert len(cuda_entries) > 0
    # Verify round-trip: _short_tag(full) maps back to the same key.
    for short, full in platform_map.items():
        assert (
...
class TestCollectNeededVariants:
    def test_single_cpu_test(self):
        ...
        py, types, cuda = collect_needed_variants(tests, _PLATFORM_MAP)
        assert py == {"3.10"}
